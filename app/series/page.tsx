...
export const metadata = { title: 'Серіали' };
...
export default async function SeriesPage({ searchParams }: Props) {
  ...
  if (trimmed) {
    const data = (await fetchTmdbPublic('/search/tv', { query: trimmed, page: String(currentPage) })) as TmdbResponse<MediaItem>;
    items = data.results.filter((i) => i.poster_path).map((i) => ({ ...i, media_type: 'tv' as const }));
    ...
    items = (data.items as MediaItem[]).filter((i) => i.poster_path).map((i) => ({ ...i, media_type: 'tv' as const }));
  ...
  const buildUrl = (p: number) => {
    ...
    return `/series?${ps.toString()}`;
  };

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-3xl font-bold text-white mb-6">📺 Серіали</h1>
      <SearchForm defaultValue={trimmed} action="/series" placeholder="Назва серіалу..." />
      {!trimmed && <Suspense><Filters type="tv" /></Suspense>}

      <div className="flex items-center justify-between mt-6 mb-4">
        <p className="text-gray-500 text-sm">
          {totalResults > 0 && <>Знайдено <span className="text-white font-medium">{totalResults.toLocaleString()}</span> серіалів</>}
        </p>
