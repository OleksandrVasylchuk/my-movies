import { getTVReviews } from '@/lib/tmdb';
...
export default async function SeriesReviewsPage({ params }: Props) {
  const { id } = await params;
  const data = (await getTVReviews(id)) as { results: Review[] };
  const reviews = data.results;

  if (reviews.length === 0) {
    return <p className="text-gray-500 text-center py-12">Рецензій поки немає.</p>;
  }
          ...
          <div key={review.id} className="p-5 bg-bg-secondary rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-bg-tertiary flex items-center justify-center text-gold text-sm font-bold flex-shrink-0">
              ...
              <div className="flex items-center gap-3">
                {review.author_details?.rating != null && (
                  <span className="text-xs text-gold font-semibold">★ {review.author_details.rating}/10</span>
                )}
