...
import { getTVCast, getImageUrl } from '@/lib/tmdb';
...
export default async function SeriesCastPage({ params }: Props) {
  const { id } = await params;
  const data = (await getTVCast(id)) as { cast: CastMember[] };
  const cast = data.cast.slice(0, 24);

  if (cast.length === 0) {
    return <p className="text-gray-500 text-center py-12">Інформація про акторів відсутня.</p>;
  }
