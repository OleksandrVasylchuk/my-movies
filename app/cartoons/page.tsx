...
export const metadata = { title: 'Мультфільми' };
...
export default async function CartoonsPage({ searchParams }: Props) {
  ...
  const buildUrl = (p: number) => {
    ...
    return `/cartoons?${ps.toString()}`;
  };

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-3xl font-bold text-white mb-6">🎨 Мультфільми</h1>
      <SearchForm defaultValue={trimmed} action="/cartoons" placeholder="Назва мультфільму..." />
      {!trimmed && <Suspense><Filters type="movie" /></Suspense>}

      <div className="flex items-center justify-between mt-6 mb-4">
        <p className="text-gray-500 text-sm">
          {totalResults > 0 && <>Знайдено <span className="text-white font-medium">{totalResults.toLocaleString()}</span> мультфільмів</>}
        </p>
